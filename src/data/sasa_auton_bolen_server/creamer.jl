const creamer_atom_radii = Dict{String, Float32}(
    "Nsp2" => 1.64,
    "Nsp3" => 1.64,
    "Csp2" => 1.76,
    "Csp3" => 1.88,
...
const PDB_ATOM_HYBRIDIZATION = Dict{String, Dict{String,String}}(
...
function creamer_atom_type(at::Atom)
    rn = resname(at)
    n = name(at)
    if haskey(PDB_ATOM_HYBRIDIZATION[rn], n)
        return PDB_ATOM_HYBRIDIZATION[rn][n]
    elseif haskey(PDB_ATOM_HYBRIDIZATION["bb"], n)
        return PDB_ATOM_HYBRIDIZATION["bb"][n]
    else
        element(at) == "H" && return "H"
    end
    throw(ArgumentError(("""\n
        Could not determine Creamer united atom type for $rn - $n
...
function creamer_sasa_restype(atoms::AbstractVector{<:PDBTools.Atom})
    ...
    sasa_atoms = sasa_particles(atoms;
        ...
        atom_radius_from_type = type -> creamer_atom_radii[type],
    )
